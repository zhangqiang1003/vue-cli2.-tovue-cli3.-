      ...
      this.$nextTick(() => {
        warmTipsApi()
          .then(res => {
            ...
            let reg = /<[^>]+>/g
            this.txtP = data.replace(reg, '')
            let regH3 = /<h3>/g
            let regP = /<p>/g
            let regPNew = `<p style="color:#75738B;text-indent:2em;line-height:22px;">`
            let regh3New = `<h3 style="margin-bottom:10px;font-weight:bold;">`
            let result = res.content.replace(regH3, regh3New).replace(regP, regPNew)
            window.localStorage.setItem('warmTip', result)
          })
      })
    }
  },
