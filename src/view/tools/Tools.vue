<template>
  <div id="ljb-tools">
    <i class="icon-logo" @click.stop.prevent="clickToOpenDebugFn()"></i>
    <span class="tip-vip-finish" v-if="tipVipFinishTxt">
      {{ $t('i18n.VIP_FINISH') }}：{{ tipVipFinishTxt }}
    </span>
    <button class="update-tip-btn"
      :class="{'mgr128': isAddClassMgr128 && isShowUpdateBtn, 'show': isShowUpdateBtn && isNewVer && isShowLoginStatus}"
...
export default {
  ...
  methods: {
          ...
          let aa = setTimeout(() => {
            sysFirmwareCheck()
              .then(data => {
                ...
                this.compatibleVer = data.compatible_version || '0.0.0'
                this.forceUpgrade = parseInt(data.force_upgrade)
    ...
    checkForceUpdateFn: function () {
      let result = compareVersion(this.compatibleVer, this.currentVer)
      if (this.forceUpgrade === 1 || !result) {
        // 需要强制升级
        this.$router.push('/check-sys-failed')
      } else {
        // 不需要强制升级
