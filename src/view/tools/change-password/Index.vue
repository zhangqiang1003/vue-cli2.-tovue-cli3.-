      ...
      <div class="wrapper">
        <label>
          <input :type="inputType"
            ...
            class="pdr"
            :class="{'error': !pass.vaildTrue}"
            @focus="inputFocus('psw')"
            @keyup.enter="changePass()"
            v-model="pass.value">
          <div class="input-error" v-if="!pass.vaildTrue">{{ $t('i18n.PASSWORD_RULE') }}</div>
          <span @click.stop.prevent="switchPasswordFn()"
            class="switch-psd"
            v-show="pass.vaildTrue">
        ...
        <label>
          <input :type="inputType"
            ...
            :class="{'error': !confirmPass.vaildTrue}"
            @focus="inputFocus('confirm')"
            @keyup.enter="changePass()"
            class="pdr"
            v-model="confirmPass.value">
