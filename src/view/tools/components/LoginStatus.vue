...
export default {
  ...
  methods: {
    ...
    memberLoginOkFn: function() {
      if (this.accStatus === 'INIT') return false
      getUserDetailsApi()
        .then(data => {
    ...
    getAccState: function(data) {
      ...
      this.userExpire = data.expireTime
      if (this.accStatus && this.accStatus !== 'INIT') {
        this.vipName = data.userName
        this.alreadyGetUserInfo = true
        this.isGetUserInfo = true
        this.isLogin = true
    ...
    watchAccStatus: function() {
      if (this.accStatus === 'INIT' && this.accStatus) {
        this.isLogin = false
        this.isGetUserInfo = false
        this.$root.Bus.$emit('busMgr128', false)
      } else {
        this.$emit('expirationTime', this.userExpire)
        let aa = setTimeout(() => {
          clearTimeout(aa)
          this.memberLoginOkFn()
        }, 7000)
  ...
  created() {
    ...
    this.$root.Bus.$on('busGetAccState', (data) => {
  ...
  beforeDestroy() {
    ...
    this.$root.Bus.$off('busGetAccState')
  }
}
</script>
