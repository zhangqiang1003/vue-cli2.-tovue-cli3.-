<template>
  <transition-group tag="ul" class="tools-nav-lists" name="fade" :class="{'opacity': isShowLists}">
    ...
    <button v-if="isShowLists" :disabled="isDisabled" class="tool-list" v-for="(item, key) in navListsArr" :key="'toolList' + key" @click="toggleRoute(item.path)">
    ...
    logout: function() {
      UserLoginApi({ action: 'logout' })
        .then(res => {
          ...
          this.isShowLists = false
          // 同时控制加速按钮的的视图变化
          this.$root.Bus.$emit('vipEndLogout')
          this.$root.Bus.$emit('busSpeedUpBtnLoginout')
          this.$root.Bus.$emit('busShowUpdateBtn', true)
          this.$root.Bus.$emit('busClickLogoutBtn')
        })
        .catch(e => {
          // todo 错误处理
        })
