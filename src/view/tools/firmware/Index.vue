<template>
  <div class="firmware-update-recovery">
    <div class="left" :class="{'blur': isShowBlur}">
      ...
      <div class="tab" :class="{'active': currentTab === 'experienceUpdate'}" @click="changeTab('experienceUpdate')">
        <div class="wrapper">
          <span>{{ $t('i18n.EXPERIENCE_VERSION') }}</span>
          <i class="arrow-wrapper">
            <arrow-incon position="right" color="blue"></arrow-incon>
          </i>
        </div>
...
export default {
  ...
  created () {
    ...
    if (status === 1) {
      this.currentTab = 'onlineUpdate'
    }
  },
  mounted() {
    this.$root.Bus.$on('showBlur', (data) => {
      this.isShowBlur = data
    })
  },
  beforeDestroy() {
    this.$root.Bus.$off('showBlur')
  }
}
</script>
