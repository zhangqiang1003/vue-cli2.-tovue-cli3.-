...
export default {
  ...
  methods: {
    ...
    busCheckPassFn: function() {
      ...
      if (this.verifyIp(this.IP.value)) {
        this.checkPass[0] = true
      } else {
        this.checkPass[0] = false
      }
      if (regular.verifyMask(this.subnetMask.value)) {
        this.checkPass[1] = true
      } else {
        this.checkPass[1] = false
      }
      if (this.verifyIp(this.gateway.value)) {
        this.checkPass[2] = true
      } else {
        this.checkPass[2] = false
      }
      if (this.verifyIp(this.DNS1.value)) {
        this.checkPass[3] = true
      } else {
        this.checkPass[3] = false
      }
    ...
    handleCheckedBtn: function(data) {
      if (data.mtu) {
        this.checked = true
        this.showInputH = true
      } else {
        this.checked = false
        this.showInputH = false
      }
    },
    emitChecked: function() {
      this.$emit('busStChecked', this.checked)
    }
  },
  watch: {
    'checked': 'emitChecked'
