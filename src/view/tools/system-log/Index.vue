...
<div class="mgb">
  <div class="system-log">
    <div class="container">
      ...
      <div class="time-tip">
        <span>{{$t('i18n.CURR_TIME')}}：{{ currTime }}</span>
        <span class="long-time">{{$t('i18n.LONG_TIME')}}：{{ longTime }}</span>
        </div>
...
export default {
  ...
  methods: {
    ...
    apiGetSysLogFn: function () {
      getSystemLogApi()
        .then(res => {
          if (Number.parseInt(res.ret) === 0) {
            this.currTime = res.date
            this.longTime = res.uptime
            this.log = res.log
